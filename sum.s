...
# Registers that we can use, x5 to x9; reason to be explained later
...
# int size = 10, sum1 = 0, sum2 = 0;
    ...
    addi x7, x0, 0      # let x7 be sum1 and set it to 0

    # for(i = 0; i < size; i++)
    #       arr1[i] = i + 1;
    addi x8, x0, 0       # let x0 be i and set it to 0
    la x9, arr1         # loading the address of arr1 to x0

loop1:
    bge x8, x5, exit1      # check if i >= size
...
loop2:
    bge x8, x5, exit2     # check if i >= size
    # we need to calculate &arr[i]
    # we need the base address of arr1
    # then, we add an offset of i * 4 to the base address
    slli x18, x8, 2      # set x18 to i * 4
...
loop3:
    bge x8, x5, exit3      # check if i >= size
    # sum1 + arr1[i]
    # need &arr1[i]
    slli x18, x8, 2     # set x18 to i * 4
    add x19, x18, x9    # add i * 4 ti the base address of arr1 and put it to x19
